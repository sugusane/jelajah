      ...
      filtered.forEach(loc => {
        ...
        card.innerHTML = `
          ...
          <div class="booth-card-footer">
            <span class="booth-status-text ${isDone ? 'done' : (isQuick ? 'quick' : 'todo')}">
              ${isDone ? '✅ Misi Selesai' : (isQuick ? '⚡ Rute Kilat' : '⏳ Belum Selesai')}
            </span>
            <button class="btn-booth-action">
              ${isDone ? 'Lihat Status ✓' : (isQuick ? 'Mulai Kilat ⚡' : 'Buka Misi ➔')}
            </button>
          </div>
        `;
